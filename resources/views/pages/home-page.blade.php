        ...
        <div class="px-2 xl:px-20 lg:px-20 md:px-15 sm:px-5">
            <div class="flex flex-col gap-8">
                <div class="flex flex-col px-4 gap-4">
                    <samp class="font-bold uppercase">PRODUCTOS DE TEMPORADA</samp>
                    <div class="flex gap-2  flex-wrap ">
                        @foreach($categorias as $item)
                        <a href="{{ route('productos.categoria', $item->id) }}" class="px-10 py-2 rounded-2xl text-black font-bold  border border-gray-400  hover:bg-black hover:text-white focus:bg-black  transition duration-300 ease-in-out selected:bg-black selected:border-none"
                            href="">
                            {{ $item->name }}
                        </a>
                        @endforeach
